...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const targetFields = String(payload.targetFields || "")
      .split(",")
      .map((value) => value.trim())
      .filter(Boolean);
    const targetFieldsOther = String(payload.targetFieldsOther || "").trim();
    const motivation = String(payload.motivation || "").trim();
    const validCvStatuses = new Set(["current", "needs_update", "not_sure"]);
    ...
    if (
      ...
      (targetFields.includes("other") && !targetFieldsOther) ||
      !payload.careerGoal ||
      !motivation ||
      !payload.privacyConsent ||
      (cv instanceof File && !validCvStatuses.has(String(payload.cvStatus || "")))
    ) {
    ...
    const profile = { applicant_id: applicant.id, institution: payload.institution || null, major: payload.major || null, graduation_year: payload.graduationYear || null, current_status: payload.currentStatus, target_role: payload.targetRole || null, target_industry: payload.targetIndustry || null, target_fields: targetFields.join(","), target_fields_other: targetFields.includes("other") ? targetFieldsOther : null, english_level: payload.englishLevel, career_goal: payload.careerGoal || null, availability: payload.availability || null, current_profile_summary: payload.currentProfileSummary || null, linkedin_url: payload.linkedin || null, updated_at: new Date().toISOString() };
